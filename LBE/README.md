...
6. **Step 6 · [Role Catalog](reference/role-catalog.md)**  
   ...
   `[Capability Catalog](reference/capability-catalog.md)` _(legacy recap)_ → `[Policy Matrix](reference/policy-matrix.md)` → `[VPD Checklist](reference/vpd-checklist.md)` → `[Audit Quick Reference](reference/audit-quick-reference.md)` → `[PostgreSQL Operations](reference/postgres-operations.md)` → `[Raw Reference Index](reference/raw/README.md)`
