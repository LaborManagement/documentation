...
flowchart TD
    ...
    Role --> Policy
    Policy --> Endpoint
    Policy --> UIAction[UI Action]
```

## Reasoning About A Request

1. **Who are you?** Validate the JWT and collect roles.
2. **What do you need?** Identify which policy the endpoint requires.
3. **Do you have access?** Check if the user's roles include the required policy.
4. **Should this button exist?** The UI hides controls unless the policy is assigned to the user's role.
...
- The policy `ADMIN_OPS_POLICY` is created to grant access to administrative operations.
- The endpoint `POST /operations/cash/open` requires the `ADMIN_OPS_POLICY`.
- The UI button "Open Cash Office" checks the authorization matrix and hides itself for users without this policy.

Just like a supervisor key, the policy is only assigned to trusted staff roles.
...
2. **Name the policy** using a descriptive pattern: `EMPLOYER_PAYMENT_LEDGER_POLICY`.
3. **Pick the roles** that should receive it.
4. **Link the endpoint** or page action to the policy.
5. **Verify with contrasting tests**: one user who should pass, one who should fail.

## Where The Data Lives

- Policies and role assignments live in PostgreSQL tables.
...
- `guides/extend-access.md` for the hands-on steps to add a new policy.
- `reference/role-catalog.md` to check which roles currently hold which policies.
